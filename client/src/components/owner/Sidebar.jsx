...
const Sidebar = ({ open = false, setOpen = ()=>{} }) => {
  ...
  return (
    <div className={`relative min-h-screen flex flex-col items-center pt-8 max-w-60 w-full border-r border-borderColor text-sm transition-transform duration-300 bg-white z-50
    ${open ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 fixed md:static left-0 top-[56px] h-[calc(100vh-56px)]`}>
      ...
      <div className='w-full' onClickCapture={()=> setOpen(false)}>
        {ownerMenuLinks.map((link, index)=>(
            <NavLink key={index} to={link.path} onClick={()=> setOpen(false)} onTouchStart={()=> setOpen(false)} className={`relative flex items-center gap-2 w-full py-3 pl-4 first:mt-6 ${link.path === location.pathname ? 'bg-primary/10 text-primary' : 'text-gray-600'}`}>
